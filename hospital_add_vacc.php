...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_vacc_btn'])) {
    if (empty($_POST['vacc_name'])) {
        ...
        $sql = " INSERT INTO `vaccines` (`vaccine_name`, `batch_no`, `doses`, `vaccine_type`, `target_age`, `m_date`, `exp_date`, `stock_status`, `hospital_id`, `batch_info`)
        VALUES ( '$vacc_name', '$vacc_batch', '$dose', '$vacc_type', '$target_age', '$m_date', '$exp_date', '$stock', '$hospital_id', '$batch_info');";
        ...
        <!-- Breadcrumb -->
            ...
            <div class="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-lg border border-gray-100 max-w-4xl mx-auto">
                ...
                <div class="flex items-center gap-4 mb-8 pb-8 border-b border-gray-100">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-darkblue">Add Vaccine</h1>
                        <p class="text-gray-500 text-sm">Enter details to Add Vaccine in inventory</p>
                    </div>
                </div>
                ...
                <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>" class="space-y-8">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="vacc_name" value="<?php echo $_POST['vacc_name'] ?? ''; ?>" placeholder="Vaccine Name" class="w-full px-5 py-4 bg-white border border-gray-200 rounded-2xl text-sm font-mano text-slate-800 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase mb-2 tracking-wider">Batch Number</label>
                                <input type="text" name="vacc_batch" value="<?php echo $_POST['vacc_batch'] ?? ''; ?>" placeholder="e.g. BATCH-2024-A1" class="w-full px-5 py-4 bg-white border border-gray-200 rounded-2xl text-sm font-mono text-slate-800 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <select name="stock" class="w-full px-5 py-4 bg-white border border-gray-200 rounded-2xl text-sm font-medium text-slate-600 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition appearance-none">
                                    ...
                                    <option value="high" <?php if(($_POST['stock'] ?? '') == 'high') echo 'selected'; ?>>In Stock (High)</option>
                                    <option value="low" <?php if(($_POST['stock'] ?? '') == 'low') echo 'selected'; ?>>Low Stock</option>
                                    <option value="out" <?php if(($_POST['stock'] ?? '') == 'out') echo 'selected'; ?>>Out of Stock</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase mb-2 tracking-wider">Manufacturing Date</label>
                                <input type="date" name="m_date" value="<?php echo $_POST['m_date'] ?? ''; ?>" class="w-full px-5 py-4 bg-white border border-gray-200 rounded-2xl text-sm font-medium text-slate-600 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase mb-2 tracking-wider">Expiry Date</label>
                                <input type="date" name="exp_date" value="<?php echo $_POST['exp_date'] ?? ''; ?>" class="w-full px-5 py-4 bg-white border border-gray-200 rounded-2xl text-sm font-medium text-slate-600 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input type="number" name="dose" value="<?php echo $_POST['dose'] ?? ''; ?>" min="1" max="12" placeholder="0" class="w-full px-5 py-4 bg-white border border-gray-200 rounded-2xl text-sm font-bold text-slate-800 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition">
                            </div>
                            <div>
                                ...
                                <select name="vacc_type" class="w-full px-5 py-4 bg-white border border-gray-200 rounded-2xl text-sm font-medium text-slate-600 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition appearance-none">
                                    ...
                                    <option value="Oral" <?php if(($_POST['vacc_type'] ?? '') == 'Oral') echo 'selected'; ?>>Oral Drops</option>
                                    <option value="Injection" <?php if(($_POST['vacc_type'] ?? '') == 'Injection') echo 'selected'; ?>>Injection</option>
                                    <option value="Nasal" <?php if(($_POST['vacc_type'] ?? '') == 'Nasal') echo 'selected'; ?>>Nasal Spray</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase mb-2 tracking-wider">Targeted Age</label>
                                <input type="text" name="target_age" value="<?php echo $_POST['target_age'] ?? ''; ?>" placeholder="e.g. 1-6 YEAR's" class="w-full px-5 py-4 bg-white border border-gray-200 rounded-2xl text-sm font-mono font-medium text-slate-600 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition">
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea rows="4" name="batch_info" placeholder="Any specific Infomation about this Vaccine for Parents..." class="w-full px-5 py-4 bg-white border border-gray-200 rounded-2xl text-sm font-medium text-slate-600 focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition resize-none"><?php echo $_POST['batch_info'] ?? ''; ?></textarea>
                    </div>
                    ...
                    <div class="pt-6 flex flex-col sm:flex-row gap-4 border-t border-gray-100">
                        ...
                        <button type="submit" name="add_vacc_btn" class="w-full sm:w-auto px-8 py-4 bg-primary text-white rounded-2xl text-sm font-bold shadow-lg shadow-blue-900/20 hover:bg-cyan-500 transition flex items-center justify-center gap-2 ml-auto">
